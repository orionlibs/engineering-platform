...
public class GetEnabledAlarmsAPIController extends APIService
{
    ...
    @Operation(
                    summary = "Retrieves enabled alarm details",
                    description = "Retrieves enabled alarm details",
                    responses = {@ApiResponse(responseCode = "200", description = "Enabled alarms details found",
                                    content = @Content(
                                                    mediaType = MediaType.APPLICATION_JSON_VALUE,
                                                    schema = @Schema(implementation = AlarmsDTO.class)
                                    ))}
    )
    @GetMapping(value = "/alarms/enabled", produces = MediaType.APPLICATION_JSON_VALUE)
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<AlarmsDTO> getEnabledAlarmsDetails()
    {
        List<AlarmModel> alarms = alarmService.getEnabledAlarms();
