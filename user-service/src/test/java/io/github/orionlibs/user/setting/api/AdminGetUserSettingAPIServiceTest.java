...
@ActiveProfiles("test")
public class AdminGetUserSettingAPIServiceTest
{
    ...
    void setup()
    {
        basePath = "http://localhost:" + port + ControllerUtils.baseAPIPath + "/users/%s/settings/%s";
        dao.deleteAll();
    ...
    @Test
    void adminGetUserSetting()
    {
        List<UserSettingsModel> settings = userSettingsService.getByUserID(user.getId().toString());
        RestAssured.baseURI = String.format(basePath, user.getId().toString(), settings.get(0).getId().toString());
        Response response = apiUtils.makeGetAPICall(headers, admin.getId().toString(), "ADMINISTRATOR");
