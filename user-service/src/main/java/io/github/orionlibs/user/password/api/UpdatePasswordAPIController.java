...
public class UpdatePasswordAPIController extends APIService
{
    ...
    public ResponseEntity<?> updateUserPassword(@Valid @RequestBody UpdatePasswordRequest requestBean, HttpServletRequest request)
    {
        boolean passwordUpdated = passwordService.update(getUserID(request), requestBean);
        if(passwordUpdated)
        {
